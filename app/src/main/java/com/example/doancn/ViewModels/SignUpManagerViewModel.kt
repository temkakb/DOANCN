...
class SignUpManagerViewModel(val account: AccountSignUp, val context: Context) : ViewModel() {
    ...
    init {
        ...
        rolepick.put(context.resources.getString(R.string.teacher), 0)
        rolepick.put(context.resources.getString(R.string.student), 1)
        var postion = 0
        for (gender in genderarray) {
            genderpick.put(gender,genderpost[postion++])
        }
    }
}