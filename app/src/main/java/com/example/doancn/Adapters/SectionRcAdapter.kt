...
class SectionRcAdapter (var listsection : List<SectionX>,
                        private val listener: OnSectionItemClickListener
): RecyclerView.Adapter<SectionRcAdapter.Sectionholder>() {
    ...
    inner class Sectionholder(itemview: View) : RecyclerView.ViewHolder(itemview){
        ...
        fun setData(sectionX: SectionX) {
            ...
            if (sectionX.numberStudent == 0) // neu so luong student cua lop = 0 it mean ng dung test diem danh
                ...
                100 - (sectionX.attendancednumbers.toFloat() / sectionX.numberStudent * 100)
            ...
            if (percent > 50) itemView.color.background =
                ContextCompat.getDrawable(context, R.drawable.bg_dangerous_section)
            else if (percent > 10) itemView.color.background =
                ContextCompat.getDrawable(context, R.drawable.bg_warning_section)
            else itemView.color.background =
                ContextCompat.getDrawable(context, R.drawable.bg_safe_section)
        }
